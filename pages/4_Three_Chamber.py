...
# 页面标题和说明
st.title("🏠 三箱实验分析 / Three Chamber Analysis")
...
# 设置路径和时间
root_directory = os.path.join(get_data_path(), 'three_chamber')
...
# 设置模型
models = {
    "ICR-DJ-Three-Chamber-V2": os.path.join(get_models_path(), "DJ-three-chamber-500px-narrow-2024-05-13/config.yaml"),
    "C57-DJ-Three-Chamber-V2": os.path.join(get_models_path(), "C57-Three-Chamber-T2-2024-05-15/config.yaml")
}
    ...
    if folder_path:
        ...
        if st.button("⚡ 处理分析结果 / Process Analysis Results", use_container_width=True):
            with st.spinner("处理中 / Processing..."):
                process_tc_files(folder_path, 0.999, 15, 35)

...
from src.core.processing.mouse_grooming_video_processing import process_grooming_files
...
# 页面标题和说明
st.title("🐁 小鼠理毛分析 / Mouse Grooming Analysis")
...
# 设置路径和时间
root_directory = os.path.join(get_data_path(), 'mouse_grooming')
...
# 设置模型
models = {
    "ICR-DJ-Grooming-V2": os.path.join(get_models_path(), "DJ-grooming-500px-narrow-2024-05-13/config.yaml"),
    "C57-DJ-Grooming-V2": os.path.join(get_models_path(), "C57-Grooming-T2-2024-05-15/config.yaml")
}
    ...
    if folder_path:
        ...
        if st.button("⚡ 处理分析结果 / Process Analysis Results", use_container_width=True):
            with st.spinner("处理中 / Processing..."):
                process_grooming_files(folder_path, 0.999, 15, 35)
